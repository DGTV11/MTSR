...
EXPANSION_PROMPT = "Generate ONE and ONLY ONE reasoning step that will aid you in responding to the query BASED on the query and the previous reasoning step(s) if any."
...
EVALUATION_PROMPT = 'Analyse the generated reasoning step strictly and directly. Your analysis MUST be based on HOW FINISHED AND COMPLETE the SUM TOTAL of ALL the generated reasoning steps are in aiding in the generation of a WELL-WRITTEN answer to the inital query, "$QUERY". Do NOT give entirely positive feedback - you MUST find AS MANY things that CAN AND MUST be improved as possible (e.g. phrasing, content, factuality). You are capable of giving strict and direct feedback that is specific and actionable, and therefore MUST do so. DO NOT REFUSE TO MAKE AN EVALUATION!!! Output an INTEGER score between [-100,+100] (i.e. from -100 to +100) between <output> tags (e.g. <output>0</output> for a final score of 0). The <output> tags ABSOLUTELY MUST ONLY BE USED TO ENCLOSE THE FINAL OUTPUT!!! Let\'s think step by step.'
