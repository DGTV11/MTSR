...
EXPANSION_PROMPT = "Generate ONE and ONE and ONLY ONE reasoning step detailing ONE HIGH-QUALITY step of your strategy to respond to the initial query BASED on the initial query and the previous reasoning step(s) if any. You MUST NOT respond directly to the initial query!"
...
MAX_ROLLOUT_ESTIMATION_PROMPT = "QUERY: \"$QUERY\"\n\nAnalyse the given query and the previous chat history (if any) and estimate how many reasoning steps you will need for a good chain of thought for that query. GIVE THE $ESTIMATION_TYPE ESTIMATE! DO NOT do the reasoning, only ESTIMATE the length of the final chain of thought in reasoning steps!\n\nHelping questions:\nIs the task familiar and factual?\nDoes the task involve ambiguity or complexity?\nWhat is my confidence in the response?\nDoes the query ask for reasoning, planning, or multi-step solutions?\n\nYou MUST not use <thoughts> tags, as they are ONLY to be used during your HIDDEN reasoning! After your reasoning, you ABSOLUTELY MUST output an NATURAL NUMBER (i.e. an integer ABOVE 0) denoting your final estimate between <output> tags (e.g. <output>1</output> for a final estimated chain of thought length of 1). The <output> tags ABSOLUTELY MUST ONLY BE USED TO ENCLOSE THE FINAL OUTPUT!!! Prompt format: \"[reasoning] Estimated chain of thought length: <output>[score]</output>\". Let's think step by step."
