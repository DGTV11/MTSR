...
EXPANSION_PROMPT = "Generate ONE and ONE and ONLY ONE HIGH-QUALITY reasoning step detailing ONE step of your strategy to prepare a final response to the initial query AND the EXECUTION of that step of your strategy. This step must be made BASED on the initial query, the previous reasoning step(s) if any, AND relevant parts of the previous chat history if any. You MUST NOT give the final response to the initial query! You MUST NOT speak to the user! You MUST keep thinking or wrap up your thought process when you have a chain of thought of a sufficient length! Never, EVER say things like \"Let me know if there's anything else I can help with.  😄\"!!! YOU DON'T GET TO DECIDE WHEN THE CHAIN OF THOUGHT WILL END (but you can wrap up the chain of thought if you deem it sufficiently long), A CHAIN OF THOUGHT GRADING PROCESS DOES!"
